  ...
  return (
    <div className="w-full max-w-xl">
      <form
        ...
        aria-label="Join the Freebug waitlist">
      ...
      <div aria-live="polite" className="mt-3 min-h-6 text-sm">
        {message && (
          <p
            className={`m-0 flex items-center gap-2 ${state === 'error' ? 'text-red-700 dark:text-red-300' : 'text-[var(--palm)]'}`}>
  ...
  return (
    <main className="page-wrap px-4 pb-16 pt-10 sm:pt-16">
      <section className="island-shell rise-in relative overflow-hidden rounded-[2rem] px-6 py-12 sm:px-12 sm:py-16 lg:px-16 lg:py-20">
        ...
        <div className="relative max-w-4xl">
          ...
          <p className="mb-9 max-w-2xl text-lg leading-8 text-[var(--sea-ink-soft)] sm:text-xl">
            Freebug explores your app, generates Playwright tests with your
            preferred AI model, and returns reproducible bugs with videos,
            traces, and accessibility evidence.
          </p>
          <WaitlistForm />
          <p className="mt-5 text-xs font-medium uppercase tracking-[0.13em] text-[var(--sea-ink-soft)]/80">
            Private beta · Bring any OpenAI-compatible model
          </p>
        </div>
      </section>
      ...
      <section
        ...
        aria-label="How Freebug works">
        {[
          {
            icon: GitPullRequest,
            number: '01',
            title: 'Connect your PR',
            copy: 'Install the GitHub App. Freebug reads the diff and targets the behavior that changed.',
          },
          {
            icon: Play,
            number: '02',
            title: 'Let the agent test',
            copy: 'A constrained AI plan drives Playwright through real user flows and accessibility checks.',
          },
          {
            icon: Video,
            number: '03',
            title: 'Review the evidence',
            copy: 'Get confirmed bugs with exact steps, screenshots, traces, videos, and stable report URLs.',
          },
          ...
          <article
            ...
            <div className="mb-8 flex items-center justify-between">
              <span className="grid h-11 w-11 place-items-center rounded-xl bg-[rgba(79,184,178,0.14)] text-[var(--lagoon-deep)]">
                <Icon size={21} aria-hidden="true" />
              </span>
              <span className="font-mono text-xs text-[var(--sea-ink-soft)]/60">
                {number}
              </span>
            </div>
      ...
      <section className="island-shell mt-6 flex flex-col gap-6 rounded-2xl p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
        <div className="flex items-start gap-4">
          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-[rgba(47,106,74,0.12)] text-[var(--palm)]">
          ...
          <div>
            ...
            <p className="mb-0 mt-1 max-w-2xl text-sm leading-6 text-[var(--sea-ink-soft)]">
              The model creates a validated action plan—not arbitrary code.
              Navigation stays same-origin, secrets stay out of prompts, and
              every finding must be backed by executed evidence.
            </p>
          </div>
        </div>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="shrink-0 rounded-full border border-[var(--line)] bg-[var(--surface-strong)] px-5 py-2.5 text-sm font-semibold no-underline transition hover:-translate-y-0.5">
          View on GitHub
        </a>
      </section>
    </main>
